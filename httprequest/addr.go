...
func getOrderTickerUrlValue(secret, date, backdate, from, to, flag, code string) url.Values {
	ret := make(url.Values)
	ret["secretStr"] = []string{secret}
	ret["train_date"] = []string{date}
	ret["back_train_date"] = []string{backdate}
	ret["tour_flag"] = []string{flag}
	ret["purpose_codes"] = []string{code}
	ret["query_from_station_name"] = []string{from}
	ret["query_to_station_name"] = []string{to}
	ret["undefined"] = []string{""}
	return ret
}
